...
def wifi_code(ssid: str, hidden: bool, authentication_type: str, password: str = None) -> str:
...
async def get_ssid_settings(aiomeraki: meraki.aio.AsyncDashboardAPI, network_id: str, network_name: str, ssids:str):
    ...
    async with meraki.aio.AsyncDashboardAPI(
        ...
        for task in asyncio.as_completed(network_tasks):
            ...
            if "wireless" not in network["productTypes"]:
                print(f"{network['name']}({network['id']}) doesn't have a wireless device")
                continue
        ...
        ssid_tasks = [
            get_ssid_settings(aiomeraki, id, name, args.ssids) for id,name in networIdNameMap.items()
        ]
